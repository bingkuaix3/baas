...
//	private static final String TABLE_TAKEOUT_FOOD = "takeout_food";
	...
	public void service(ServletRequest request, ServletResponse response) throws ServletException {
		try {
			...
			} else if ("getOrderCount".equals(action)) {
	...
	private static void getOrderCount(ServletRequest request, ServletResponse response) throws SQLException, IOException, NamingException {
		String sql = "SELECT COUNT(ord.fID) AS orderCount "
				+ " FROM takeout_order ord ";
		JSONObject json = new JSONObject();
		Connection conn = Util.getConnection(DATASOURCE_TAKEOUT);
		try {
			int count = Integer.parseInt(Util.getValueBySQL(conn, sql, null).toString());
